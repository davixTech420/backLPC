 ...
 exports.validarUsuario = async (req,res) => {
  try{
    ...
    const mailOptions = {
      ...
      to: email,
      subject: 'Crear Cuenta En Datech LPC',
      html: `<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="container">
        <h1>Crear Cuenta</h1>
        <p>Hola,</p>
        <p>Recibimos una solicitud para crear una nueva cuenta en DATECH LPC. Haz clic en el botón de abajo para crear la cuenta:</p>
...
exports.registrarConfirmado = async (req, res) => {
  const token = req.params.token;
  const decoded = jwt.verify(token, process.env.JWT_SECRET);
  const { nombre, apellido, tipIdentidad, identificacion, telefono, email, password, estado, role } = decoded;
  try {
